...
def n_most_recent(path, n, curr):
    most_recent = []
    prev = None
    i = len(path) - 1
    while len(most_recent) < n and i > 0:
        node = path[i]
        if node != prev and node != curr:
            most_recent.append(node)
            prev = node
        i -= 1
            
    return most_recent
    
def repair(G, pheromone_add, pheromone_decay, explore1, explore2, strategy='uniform', \
            ...
            decay_type='linear', node_queue_lim=1, edge_queue_lim=1, memory_lim=1):
    """ """
    
    graph_name = G.graph['name']
    nests = G.graph['nests']
    
    out_items = ['repair_sandbox', strategy, graph_name, decay_type]
    if backtrack:
    ...
    pher_str = "%d, %f, %f, " % (num_ants, explore2, pheromone_decay)
        ...
        while steps <= max_steps:   
            ...
            for u, v in G2.edges():
                for direction in ['forwards', 'backwards']:
                    ...
                    while i < min(eqlim, len(edge_queue)):
...
def main():
    ...
    parser.add_argument("-e2", "--explore2", type=float, dest="explore2", default=None, \
                        help="search explore probability")
    ...
    parser.add_argument('-ml', '--memory_limit', type=int, dest='memory_lim', default=1)
    ...
    repair(G, pheromone_add, pheromone_decay, explore, explore2, strategy, num_ants, max_steps,\
               num_iters, print_graph, video, frames, video2, cost_plot, backtrack, \
               decay_type, node_queue_lim, edge_queue_lim, memory_lim)
