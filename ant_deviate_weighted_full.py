...
def weighted_full_grid():
    ...
    num_plants = random_plant_labels(G)
    num_plants = float(num_plants + 1)
    ...
    for i, (u,v) in enumerate(G.edges()):
        ...
        if u[1] == 5 and v[1] == 5:
            ...
            edge_width.append(1)
        plant = G[u][v]['plant']
        edge_color.append(plant / num_plants)
...
def random_plant_labels(G):
    G2 = G.copy()
    unlabelled_edges = set(G.edges())
    plant_label = 1
    while len(unlabelled_edges) > 0:
        spanning_tree = nx.minimum_spanning_tree(G2)
        for u, v in spanning_tree.edges():
            G[u][v]['plant'] = plant_label
            G2.remove_edge(u, v)
            try:
                unlabelled_edges.remove((u, v))
            except:
                try:
                    unlabelled_edges.remove((v, u))
                except:
                    pass
            plant_label += 1
    return plant_label
...
def deviate(G, num_iters, num_ants, pheromone_add, pheromone_decay, explore_prob, \
    ...
    data_file = open('ant_deviate_weighted_full.csv' % max_steps, 'a')
    pruning_file = open('ant_deviate_weighted_full_pruning.csv' % max_steps, 'a')
    path_pruning_file = open('ant_deviate_weighted_full_path_pruning.csv', 'a')
    ...
    for iter in xrange(num_iters):
        ...
        if cost_plot:
            figname = "pruning/pruning_weighted_full%d_e%0.2fd%0.2f" % (max_steps, explore_prob, pheromone_decay)
