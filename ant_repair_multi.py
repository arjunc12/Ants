...
def init_graph(G):
    ...
    for i, (u,v) in enumerate(sorted(G.edges())):
        for idx in G[u][v]:
            G[u][v][idx]['weight'] = MIN_PHEROMONE
            
            G[u][v][idx]['forwards'] = tuple(sorted((u, v)))
            G[u][v][idx]['forwards_queue'] = []
            
            G[u][v][idx]['backwards'] = tuple(sorted((u, v))[::-1])
            G[u][v][idx]['backwards_queue'] = []
...
def total_edge_weight(G, u, v):
    total_wt = 0
    for idx in G[u][v]:
        total_wt += G[u][v][idx]['weight']
    return total_wt
...
def pheromone_cost(G):
    ...
    G2 = nx.Graph()
    for u, v in G.edges_iter():
        if total_edge_weight(G, u, v) > MIN_DETECTABLE_PHEROMONE:
            G2.add_edge(u, v)
...
def pheromone_dead_end(G, curr, prev):
    ...
    for n in G.neighbors(curr):
        if n != prev and total_edge_weight(G, curr, n) > MIN_DETECTABLE_PHEROMONE:
            return False
    return True
...
def check_queues(G, queued_nodes, queued_edges, num_ants, verbose=False):
    ...
    for edge_id in sorted(queued_edges):
        ...
        for direction in directions:
            for idx in G[u][v]:
                queue = G[u][v][idx][direction + '_queue']
                if verbose:
                    print "queued edge " + direction, edge_id, (u, v), idx, queue
...
def max_neighbors(G, source, prev=None):
    ...
    for candidate in candidates:
        wt = total_edge_weight(G, source, candidate)
        if wt <= MIN_DETECTABLE_PHEROMONE:
            continue
        elif wt > max_wt:
            max_wt = wt
...
def print_queues(G):
    ...
    for u, v in sorted(G.edges()):
        for idx in G[u][v]:
            if len(G[u][v][idx]['forwards_queue']) + len(G[u][v][idx]['backwards_queue']) > 0:
                print u, v
            if len(G[u][v][idx]['forwards_queue']) > 0:
                print 'forwards', G[u][v][idx]['forwards_queue']
            if len(G[u][v][idx]['backwards_queue']) > 0:
                print 'backwards', G[u][v][idx]['backwards_queue']
...
def repair(G, pheromone_add, pheromone_decay, explore_prob, strategy='uniform', \
    ...
    out_items = ['repair_multi', strategy, graph_name, decay_type]
    if backtrack:
    ...
    for iter in xrange(num_iters):    
        ...
        for u, v in init_path:
            G[u][v][0]['weight'] += pheromone_add * INIT_WEIGHT_FACTOR
            if decay_type == 'linear':
                G[u][v][0]['units']+= [pheromone_add] * INIT_WEIGHT_FACTOR
