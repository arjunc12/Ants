...
def deviate(G, num_iters, num_ants, pheromone_add, pheromone_decay, explore_prob, \
    ...
    data_file = open('ant_deviate_weighted_simple%d.csv' % max_steps, 'a')
    pruning_file = open('ant_deviate_weighted_simple_pruning.csv', 'a')
    path_pruning_file = open('ant_deviate_weighted_simple_path_pruning.csv', 'a')
    ...
    for iter in xrange(num_iters):
        ...
        while steps <= max_steps:
            ...
            for j in xrange(num_ants):
                ...
                if explore[j]:
                    ...
                    if curr == origins[j]:
                        next, ex = next_edge(G, curr, explore_prob=explore_prob, prev=None)
                    else:
                    ...
                    plant1 = G[prev][curr]['plant']
                    plant2 = G[curr][next]['plant']
                    switch_plant = int(plant1 != plant2)
                    
                    G2[curr][next]['weight'] += pheromone_add * (1 - (0.5 * switch_plant))
        ...
        if cost_plot:
            figname = "pruning/pruning_weighted_simple%d_e%0.2fd%0.2f" % (max_steps, explore_prob, pheromone_decay)
...
def main():
    ...
    #print "show"
    #PP.show()
    #PP.savefig("fig_weighted_simple.pdf")
    #PP.close()
