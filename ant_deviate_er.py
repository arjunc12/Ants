...
def deviate(G,num_iters, num_ants, pheromone_add, pheromone_decay, print_path=False, print_graph=False, video=False, nframes=200, explore_prob=0.1, max_steps=3000):
    ...
    data_file = open('ant_deviate_er%d.csv' % max_steps, 'a')
    ...
    for iter in xrange(num_iters):
        ...
        max_cost = 0
        while i <= max_steps:
            max_cost = max(max_cost, pheromone_cost(G))
            G2 = G.copy()
...
#                 before_paths = max(before_paths, pheromone_connectivity(G2, nest, target))
        ...
        if print_graph:        
            color_graph(G, 'g', (pheromone_add / max_weight), "graph_after_er%d_e%0.2fd%0.2f" % (max_steps, explore_prob, pheromone_decay))
            print "graph colored"
        ...
        if video:    
            ...
            ani.save("ant_deviate_er" + str(iter) + ".mp4")
        ...
        success_rate = float(successful_walks) / (successful_walks + failed_walks)
        ...
        if video:    
            ...
            ani.save("ant_deviate_er" + str(iter) + "a.mp4")
