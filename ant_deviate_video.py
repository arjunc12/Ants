...
def deviate(G,num_iters, num_ants, pheromone_add, pheromone_decay, explore_prob, max_steps, \
    ...
    for iter in xrange(num_iters):
        ...
        if print_graph:        
            color_graph(G, 'g', (pheromone_add / max_weight), "graph_after%d_e%0.2fd%0.2f" % (max_steps, explore_prob, pheromone_decay))
            print "graph colored"
        ...
        costs = PP.array(costs)
        if cost_plot:
            figname = "pruning/pruning%d_e%0.2fd%0.2f" % (max_steps, explore_prob, pheromone_decay)
